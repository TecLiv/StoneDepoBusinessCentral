report 50102 "Sales - Invoice-Packing List"
{
    // version DSJS01,DSMS02

    DefaultLayout = RDLC;
    RDLCLayout = './Sales - Invoice-Packing List.rdlc';
    WordLayout = './Sales - Invoice-Packing List.docx';
    Caption = 'Sales - Invoice-Packing List';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(DocumentTitle_Lbl; 'PACKING-SLIP' + ' |  ' + "No.")
            {
            }
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(Length_Line; Line."Length (mm)")
                {
                }
                column(Width_Line; Line."Width (mm)")
                {
                }
                column(Height_Line; Line.Height)
                {
                }
                column(Cubage_Line; Line.Cubage)
                {
                }
                column(AvgQtyperAddBaseUOM_Line; Line."SQM/Unit")
                {
                }
                column(QuantityAddBase_Line; Line."Quantity (Add. Base) (SLABS)")
                {
                }
                dataitem(DataItem1000000000; "Value Entry Relation")
                {
                    dataitem(DataItem1000000002; "Value Entry")
                    {
                        DataItemLink = "Entry No." = FIELD ("Value Entry No.");
                        dataitem(DataItem1000000009; "Item Ledger Entry")
                        {
                            DataItemLink = "Entry No." = FIELD ("Item Ledger Entry No.");
                            column(TrackingLotNo_ItemLedgerEntry; "Tracking Lot No.")
                            {
                            }
                            column(LotNo_ItemLedgerEntry; "Lot No.")
                            {
                            }
                            column(QuantityAddBaseSLABS_ItemLedgerEntry; "Quantity (Add. Base) (SLABS)")
                            {
                            }
                            column(QtyinSQM_ItemLedgerEntry; "Qty. in SQM")
                            {
                            }
                            column(Lengthmm_ItemLedgerEntry; "Length (mm)")
                            {
                            }
                            column(Widthmm_ItemLedgerEntry; "Width (mm)")
                            {
                            }
                            column(Thickness_Line; ItemRec."Thickness (mm)")
                            {
                            }
                            ...
                            column(ColourShade_ItemLedgerEntry; "Colour Shade")
                            {
                            }
                            ...
                            begin
                                ItemDescription := '';
                                IF ItemRec.GET(DataItem1000000009."Item No.") THEN
                    ...
                    begin
                        ...
                        DataItem1000000000.SETRANGE("Source RowId", SalesInvoiceRowID);
                    end;
                }
                ...
                begin
                    ...
                    SalesInvoiceRowID := RowID1;

                    //ERROR('..%1...',SalesInvoiceRowID);
                end;
    ...
    var
        ...
        PostedShipmentDateLbl: Label 'Outing Date';
        SalesInvLineDiscLbl: Label 'Discount %';
        SalesInvoiceLbl: Label 'TAX Invoice';
    ...
    begin
        WITH SalesInvoiceHeader DO BEGIN
            ...
            //FormatDocument.SetPaymentMethod(PaymentMethod, "Payment Method Code"); Dynamics
    ...
    local procedure RowID1(DocumentNo: Code[20]; LineNumber: Integer): Text[250]
    var
        ItemTrackingMgt: Codeunit "Item Tracking Management";
    begin
        EXIT(ItemTrackingMgt.ComposeRowID(DATABASE::"Sales Invoice Line",
            0, DocumentNo, '', 0, LineNumber));
    end;
}
